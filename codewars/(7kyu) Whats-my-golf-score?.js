...
Both strings will be of length 18, and each character in the string will be a number 
...
function golfScoreCalculator(parList, scoreList){
  ...
  for( let i = 0; i < scoreList.length; i++){
    let difference = parseInt(scoreList[i]) - parseInt(parList[i])
    sum.push(difference)    
    
  }
  
  
  let finalSum = sum.reduce((a,b) => a + b)
  return finalSum;
